...
export function CallControls(props: { session: SessionItem }) {
	...
	return (
		<Box css={{ display: "flex", gap: 2, flexDirection: "column", alignItems: "center", "padding-top": 12 }}>
			<Box css={{ "font-weight": "bolder" }}>{formatPhoneNumber(titlePhone())}</Box>
			<Box>{directionTranslate[props.session.direction]}</Box>
			<Box css={{ "font-size": "0.8em" }}>{sessionStateTranslate[props.session.state]}</Box>
			<Box css={{ fontSize: "0.8em" }}>Длительность: {formatSeconds(props.session.duration)}</Box>

			<Box css={{ "margin-top": "24px", display: "flex", gap: 12, alignItems: "center" }}>
